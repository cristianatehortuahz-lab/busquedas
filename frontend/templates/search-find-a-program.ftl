...
<#--
  ============================================================================
  🌐 HUB VIVO - Plantilla de Búsqueda Facetada (Programa Académico)
  ============================================================================
  Renderizada por `PagedSearchControllerFaceted` en el endpoint `/find-a-program`.
  Filtra por ontología: `http://vivoweb.org/ontology#vitroClassGrouporganizations`

  ⚠️ VARIABLES INYECTADAS ESPERADAS:
  - `querytext`: (String) El término; puede incluir el prefijo `acNameStemmed:` del autocompletado.
  - `facets`: (List<SearchFacet>) Conteos Solr por modalidad, nivel y departamento.
  - `individuals`: (List) Resultados de tipo `vivo:Program`.
  - `pagingLinks`: (List) Paginación AJAX.

  📝 PROCESAMIENTO DE QUERYTEXT (LÍNEA 7):
  Esta plantilla asigna `textShow = querytext?replace('acNameStemmed:','')` para
  mostrar sólo el texto limpio en el encabezado `<h2>`. Esto es diferente al
  Supervisor/Partner (que muestran `querytext` directamente).

  ⚠️ AVISO HISTÓRICO:
  Esta plantilla carga `dynamic-filters2.js` (no v4). Revisar si se debe
  actualizar a `dynamic-filters4.js` para mantener paridad funcional.
  ============================================================================
-->
     ...
     <div class="search_program">
       ...
       <form id="search-program" class="js-search-form" action="${urls.base}/find-a-program" method="GET">
         ...
         <div class="search_program-box">
            ...
            <input type="text" id="facets-querytext"  name="querytext" autocomplete="off" class="search_program-input-w100 js-query-text" value="${querytext!?html}"/>
