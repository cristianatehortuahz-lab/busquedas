...
<#-- 
  ============================================================================
  🌐 HUB VIVO - Plantilla de Búsqueda Facetada (Supervisor)
  ============================================================================
  Esta plantilla es renderizada por el servlet Java `PagedSearchControllerFaceted`.
  
  ⚠️ VARIABLES INYECTADAS ESPERADAS:
  - `querytext`: (String) El término de búsqueda introducido por el usuario.
  - `facets`: (List<SearchFacet>) Los conteos y campos devueltos por SolrJ.
  - `individuals`: (List) Los resultados de búsqueda a renderizar.
  - `pagingLinks`: (List) Enlaces de paginación construidos por el backend.
  
  ⚠️ INTEGRACIÓN JAVASCRIPT:
  - El formulario `<form class="js-search-form">` es capturado por `dynamic-filters4.js`
    para convertir las navegaciones en peticiones AJAX.
  - Los checkboxes generados llevan la clase `.js-checkbox-facet` para auto-construcción.
  ============================================================================
...
<div class="contentsBrowseGroup clear clearfix">
    ...
    <div class="search_supervisor">
      ...
      <form id="search-supervisor" class="js-search-form" action="${urls.base}/find-a-supervisor" method="GET">
        ...
        <div class="search_supervisor-box">
          <h4 class="search_supervisor-title">${i18n().find_supervisor}</h4>
          <#--  textbox con palabra de busqueda  -->
          <input type="text" id="facets-querytext" name="querytext" autocomplete="off"
            class="search_supervisor-input-w100 js-query-text" value="${querytext!?html}" />


          <#--  checkbox para seleccionar entre persona o palabra clave  -->
        ...
        <script
          type="text/javascript"> var urlBaseForFilterSearch = '${urls.base}' + '/find-a-supervisor'
        </script>
      </form>
    </div>
  </#if>
  ...
  <div class="search_results js-results-container">
    ...
    <div class="search_supervisor-box sort-search-box">
      <h4 class="search_supervisor-title">${i18n().sortBy}</h4>
      <select id="sort" name="sort" class="search_supervisor-input-w100 search_supervisor-select js-search">
        <#if sort?? && sort=="numTutoredTheses|DESC">
          <option value="numTutoredTheses|DESC" selected>"${i18n().relevance}"</option>
        <#else>
          <option value="numTutoredTheses|DESC" selected>"${i18n().relevance}"</option>
        </#if>
